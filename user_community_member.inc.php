...
<div id="friends" style="display: inline-block;vertical-align: top; width: 200px;background-color: #eee;">
	<?php   #this part is for community memebers
	...
	<a href="<?=ROOT?>profile/<?php echo $FRIEND_ROW['userid']; ?>">
		...
		<?php 
			...
			if($FRIEND_ROW['online'] > 0){
				...
				if(($current_time - $online) < $threshold){
					...
					$online = "Last seen: <br>" . Time::get_time(date("Y-m-d H:i:s",$online));  #here it shows the last time when user has enter website
				}
			}
		?>
		...
		<?php if(community_access($_SESSION['communitx_userid'],$community_data,'admin')):?>
			<br style="clear: both;">
			<a href="<?=ROOT?>community/<?=$community_data['userid']?>/members?remove=<?=$FRIEND_ROW['userid']?>">
				<input id="post_button" type="button" value="Remove" style="font-size:11px;margin-right:10px;background-color: #916e1b;width:auto;">
			</a>
			<a href="<?=ROOT?>community/<?=$community_data['userid']?>/members?edit_access=<?=$FRIEND_ROW['userid']?>">
