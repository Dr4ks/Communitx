<?php 
	...
	include("classes/autoload.php");      #this part is for deleting posts and comments
	$image_class = new Image();

	$login = new Login();
	$user_data = $login->check_login($_SESSION['communitx_userid']);      #first user login is checked using session global variable
	...
	if(isset($URL[1])){
		...
		if($URL[1] == "msg")    #if any problem happens such as user doesn't have privileges to delete post then this message is displayed on the screen
		{
	...
	if($ERROR == "" && $_SERVER['REQUEST_METHOD'] == "POST"){
		...
		if($URL[1] == "msg")
		{
			$msg_class->delete_one($_POST['id']);     #if everything is right then post is successfully deleted
