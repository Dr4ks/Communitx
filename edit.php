<?php 
	...
	$login = new Login();
	$user_data = $login->check_login($_SESSION['communitx_userid']);  #if user wants to edit post first user's priviles are checked 
	...
	if(isset($URL[1])){
		 ...
		 if(!$ROW){
		 	...
		 	if($ROW['userid'] != $_SESSION['communitx_userid']){    #if user wants to delete post first user's priviles are checked 

		 		$ERROR = "Accesss denied! you cant delete this file!";  #if user doesn't have privileges to delete post then error message appers
		 	}
		 }
