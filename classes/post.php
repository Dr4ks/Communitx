...
class Post      #this class contains all the functions related to posts
{
	private $error = "";
	...
	public function create_post($userid, $data, $files, $owner = 0)
	{
		...
		if(!empty($data['post']) || !empty($files['file']['name']) || isset($data['is_profile_image']) || isset($data['is_cover_image']))
		{
			...
			if(isset($data['is_profile_image']) || isset($data['is_cover_image']))   #here profile image is checked if it is set or not
			{
			...
			if($this->error == ""){    #if there is no error then function continues to create postid
				...
				if(isset($data['parent']) && is_numeric($data['parent'])){
					...
					if(is_array($mypost) && $mypost['userid'] != $userid){
						...
						//add notification
						//add_notification($_SESSION['communitx_userid'],"comment",$mypost);
					}

					$sql = "update posts set comments = comments + 1 where postid = '$parent' limit 1";
					$DB->save($sql);     #here database is updated
				}
	...
	public function get_posts($id,$post_type = "profile")  #This function helps us to get all posts which are related to specific userid
	{
	...
	public function i_own_post($postid,$communitx_userid)
	{
		...
		$result = $DB->read($query);
  		...
  		if(is_array($result)){
  			...
  			if($result[0]['userid'] == $communitx_userid){
	...
	public function like_post($id,$type,$communitx_userid){
			...
			if(is_array($result)){
				...
				if(!in_array($communitx_userid, $user_ids)){

					$arr["userid"] = $communitx_userid;
					...
					if($type != "user"){
						...
						//add notification
						//add_notification($_SESSION['communitx_userid'],"like",$single_post);
					}

				}else{

					$key = array_search($communitx_userid, $user_ids);
					unset($likes[$key]);
	...
	private function create_postid()     #this function creates random postid
	{
