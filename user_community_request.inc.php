...
<div id="friends" style="display: inline-block; width: 200px;background-color: #eee;">
	<?php  #this section is for user requests

		$image = "images/user_male.jpg";
		if($FRIEND_ROW['gender'] == "Female") #here deafult images are given to users according to their gender
		{
	...
	<a href="<?=ROOT?>profile/<?php echo $FRIEND_ROW['userid']; ?>">
		...
		?>
		<a href="<?=ROOT?>community_request_accept/<?=$community_data['userid']?>/<?=$FRIEND_ROW['userid']?>/decline">
			<input id="post_button" type="button" value="Decline" style="float:right;margin-right:10px;background-color: #916e1b;width:auto;">
		</a>
		<a href="<?=ROOT?>community_request_accept/<?=$community_data['userid']?>/<?=$FRIEND_ROW['userid']?>/accept">
